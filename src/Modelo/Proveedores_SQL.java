...
public class Proveedores_SQL {
    ...
    public boolean registrarProveedores(Proveedores proveedores){
        String SQL = "INSERT INTO proveedores(cuit_prov, nom_prov, direc_prov, telf_prov) VALUES (?,?,?,?)";
        try {
            con = conectar.getConnection();
            pst = con.prepareStatement(SQL);
            pst.setString(1, proveedores.getCuit());
    ...
    public boolean modificarProveedores (Proveedores proveedores){
        ...
        try {
            ...
            pst.setString(4, proveedores.getCuit());
            pst.execute();
            return true;
        } catch (Exception e) {
            System.out.println(e.toString());
